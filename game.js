...
function buildAssetCandidates(src){
  ...
  let scriptDir = '';
  try {
    if (scriptUrl) scriptDir = new URL('.', scriptUrl).href;
  } catch (_) {}

  const bases = [];
  if (window.__IRONVEIL_ASSET_BASE__) bases.push(String(window.__IRONVEIL_ASSET_BASE__).replace(/\/?$/, '/'));
  if (scriptDir){
    bases.push(new URL('./assets/', scriptDir).href);
    bases.push(new URL('../assets/', scriptDir).href);
  }
  bases.push('assets/');
  bases.push('./assets/');
  ...
  const seen = new Set();
  const out = [];
  const variants = [clean, bare, 'assets/' + bare];
  for (const v of variants){
    for (const base of bases){
      let candidate;
      if (/^(https?:|blob:)/i.test(base)) candidate = new URL(v, base).href;
      else candidate = (base + v).replace(/([^:]\/)\/+/g, '$1');
    ...
    return new Promise((resolve) => {
      ...
      function tryNext(){
        if (i >= candidates.length){
          console.warn('Asset failed to load:', name, candidates);
          cache[name] = null;
          loadedCount++;
          resolve(null);
          return;
...
function resizeCanvas(){
  ...
  camera.scale  = Math.max(fitAll, Math.min(desired, 1.4));
  if (w > h){
    if (h <= 560) camera.scale = Math.max(fitAll, Math.min(camera.scale, 0.92));
    else camera.scale = Math.max(fitAll, Math.min(camera.scale, 1.02));
  }
}
...
function updateCamera(){
  ...
  const isLandscape = camera.viewW > camera.viewH;
  const isCompactLandscape = isLandscape && camera.viewH <= 560;
  const sideBias = isLandscape ? TILE_SIZE * (isCompactLandscape ? 1.6 : 1.1) : 0;
  ...
  if (follow){
    ...
    if (sideBias) tx += (state.turn === 'player') ? -sideBias : sideBias;
  } else if (sideBias){
    tx += (state.turn === 'player') ? -sideBias : sideBias;
  }
...
canvas.addEventListener('mousemove', (ev) => {
  ...
  if (typeof updateInspectors === 'function') updateInspectors();
});
...
function selectUnit(unitId){
  ...
  const entriesNow = enemiesInRange(u);
  if (entriesNow.length > 0){
    flashLog(`${name} selected — foe in reach!`);
...
function updateUI(){
  ...
  // Attack button — when the active hero has any foe in their current range.
  // Lets the player engage even if they were already adjacent from a prior
  // turn (the in-move auto-popup only fires after a fresh movement).
...
function updateInspectors(){
  const activePanel = $id('inspector-active');
  const nameEl  = $id('insp-name');
  const hpEl    = $id('insp-hp');
  const hpFill  = $id('insp-hp-fill');
  const dmgEl   = $id('insp-dmg');
  const rangeEl = $id('insp-range');
  const k4El    = $id('insp-4k');
  const v4El    = $id('insp-4v');

  if (activePanel && nameEl){
    const u = state.activeUnit ? state.units[state.activeUnit] : null;
    if (u && u.alive){
      activePanel.dataset.side = u.side;
      nameEl.textContent = u.type === 'mage' ? 'MAGE' : 'FIGHTER';
      if (hpEl)   hpEl.textContent   = `${u.hp} / ${u.maxHp}`;
      if (hpFill) hpFill.style.width = (u.hp / u.maxHp * 100) + '%';
      if (dmgEl)   dmgEl.textContent   = String(u.dmg);
      if (rangeEl) rangeEl.textContent = String(u.range);
      if (u.type === 'mage'){
        if (k4El) k4El.textContent = 'HEAL';
        if (v4El) v4El.textContent = String(u.heal);
      } else {
        if (k4El) k4El.textContent = 'DEF';
        if (v4El) v4El.textContent = u.passiveHalf !== false && UNIT_DEFS.fighter.passiveHalf ? '½' : '—';
      }
    } else {
      activePanel.dataset.side = 'none';
      nameEl.textContent = '—';
      ...
      if (k4El) k4El.textContent = 'HEAL';
      if (v4El) v4El.textContent = '—';
    }
  }

  // Terrain / move / defense panel
  const moveEl    = $id('insp-move');
  const terrainEl = $id('insp-terrain-name');
  const defenseEl = $id('insp-defense');
  if (moveEl){
    if (state.turn === 'player' && state.activeUnit && state.subPhase === 'action'){
      const used = state.unitsActedThisTurn.has(state.activeUnit) ? state.dice : 0;
      moveEl.textContent = `${state.dice - used} / ${state.dice}`;
    } else if (state.dice > 0){
      moveEl.textContent = `— / ${state.dice}`;
    } else {
      moveEl.textContent = '— / —';
    }
  }
  if (terrainEl){
    if (state.hoverTile){
      ...
      if (isBlocked(x, y)){
        const t = state.blockedTypes.get(key(x,y));
        terrainEl.textContent = t === 'stone' ? 'Rubble' : t === 'bush' ? 'Brush' : 'Blocked';
      } else {
        terrainEl.textContent = 'Normal';
      }
    } else {
      terrainEl.textContent = 'Normal';
    }
