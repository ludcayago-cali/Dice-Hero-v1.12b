...
const btnZoomReset = $id('btn-zoom-reset');
...
function setZoomMultiplier(mult){
  camera.userZoom = clamp(mult, camera.minScale / Math.max(0.001, camera.baseScale), camera.maxScale / Math.max(0.001, camera.baseScale));
  camera.scale = clamp(camera.baseScale * camera.userZoom, camera.minScale, camera.maxScale);
  const game = document.getElementById('game');
  if (game) game.classList.add('inspecting');
}
function stepZoom(dir){ setZoomMultiplier(camera.userZoom * (dir > 0 ? 1.14 : 1/1.14)); }
...
function resizeCanvas(){
  ...
  const w = Math.max(stage ? stage.clientWidth : 0, window.innerWidth || 0);
  const h = Math.max(stage ? stage.clientHeight : 0, window.innerHeight || 0);
  if (w <= 0 || h <= 0) return;
  ...
  // Scale to fit tiles nicely. On tiny phones, show ~6-8 tiles across; on desktop show full 12.
  // Choose scale so at least 6 tiles are visible on the narrowest axis.
  const minTilesVisible = 6;
  const maxTilesVisible = GRID_SIZE;   // never zoom out past full grid
  const byWidth  = w / (minTilesVisible * TILE_SIZE);
  const byHeight = h / (minTilesVisible * TILE_SIZE);
  const maxScale = Math.max(byWidth, byHeight);            // "big tiles"
  // But don't allow us to fit fewer than full grid on large screens:
  const fitAllX = w / (maxTilesVisible * TILE_SIZE);
  const fitAllY = h / (maxTilesVisible * TILE_SIZE);
  const fitAll  = Math.min(fitAllX, fitAllY);
  // Final scale = max(fitAll, clamp of byWidth/byHeight).
  // Cap upper limit so sprites don't become giant on desktop.
  const desired = Math.min(maxScale, 1.4);
  camera.baseScale = Math.max(fitAll, Math.min(desired, 1.4));
  camera.minScale = Math.max(fitAll * 0.72, 0.48);
  camera.maxScale = Math.max(camera.baseScale * 2.4, 1.6);
  camera.userZoom = clamp(camera.userZoom || 1, camera.minScale / camera.baseScale, camera.maxScale / camera.baseScale);
  camera.scale = clamp(camera.baseScale * camera.userZoom, camera.minScale, camera.maxScale);
}
...
function getSideFocusUnit(side){
  const units = Object.values(state.units).filter(u => u.alive && u.side === side);
  if (!units.length) return null;
  const avgX = units.reduce((s,u) => s + u.x + (u.renderOffsetX || 0), 0) / units.length;
  const avgY = units.reduce((s,u) => s + u.y + (u.renderOffsetY || 0), 0) / units.length;
  return { x: avgX, y: avgY };
}

function ensureCanvasHealthy(){
  if (!canvas.parentElement) return;
  if (canvas.width === 0 || canvas.height === 0 || !camera.viewW || !camera.viewH){
    resizeCanvas();
  }
}
...
function updateCamera(){
  ...
  if (follow){
    ...
    const focus = getSideFocusUnit(state.turn) || getSideFocusUnit('player') || getSideFocusUnit('enemy');
    if (focus){
      tx = focus.x * TILE_SIZE + TILE_SIZE / 2;
      ty = focus.y * TILE_SIZE + TILE_SIZE / 2;
    }
  }
...
function beginPointer(id, clientX, clientY){
  gesture.pointers.set(id, { x: clientX, y: clientY, startX: clientX, startY: clientY });
  if (gesture.pointers.size === 1){
    gesture.dragging = false;
    gesture.suppressTap = false;
    gesture.dragStart = { x: clientX, y: clientY, panX: camera.panX, panY: camera.panY };
  } else if (gesture.pointers.size === 2){
    const pts = [...gesture.pointers.values()];
    gesture.pinch = true;
    gesture.lastDist = distance(pts[0], pts[1]);
...
function movePointer(id, clientX, clientY){
  const p = gesture.pointers.get(id);
  if (!p) return;
  ...
  if (gesture.pointers.size >= 2){
    ...
    if (gesture.lastDist){
      ...
      if (Number.isFinite(ratio) && ratio > 0){
        setZoomMultiplier(camera.userZoom * ratio);
      }
    }
    if (gesture.lastMid){
      camera.panX -= (mid.x - gesture.lastMid.x) / camera.scale;
      camera.panY -= (mid.y - gesture.lastMid.y) / camera.scale;
    }
  ...
  if (gesture.dragStart){
    const dx = clientX - gesture.dragStart.x;
    const dy = clientY - gesture.dragStart.y;
    if (!gesture.dragging && Math.hypot(dx, dy) > 10){
      gesture.dragging = true;
      gesture.suppressTap = true;
      const game = document.getElementById('game');
      if (game) game.classList.add('inspecting');
    ...
    if (gesture.dragging){
      camera.panX = gesture.dragStart.panX - dx / camera.scale;
      camera.panY = gesture.dragStart.panY - dy / camera.scale;
...
function endPointer(id, clientX, clientY, originalEvent){
  const p = gesture.pointers.get(id);
  if (p && !gesture.dragging && !gesture.pinch && !gesture.suppressTap){
    handleCanvasPointer(originalEvent, true);
  }
  gesture.pointers.delete(id);
  if (gesture.pointers.size < 2){
    gesture.pinch = false;
    gesture.lastDist = 0;
    gesture.lastMid = null;
  }
...
canvas.addEventListener('mousemove', (ev) => {
  if (state.phase !== 'playing' || gesture.dragging || gesture.pinch) return;
...
canvas.addEventListener('mousedown', (ev) => {
  ...
  if (ev.cancelable) ev.preventDefault();
  beginPointer('mouse', ev.clientX, ev.clientY);
});
window.addEventListener('mousemove', (ev) => {
  if (!gesture.pointers.has('mouse')) return;
  movePointer('mouse', ev.clientX, ev.clientY);
});
window.addEventListener('mouseup', (ev) => {
  if (!gesture.pointers.has('mouse')) return;
  endPointer('mouse', ev.clientX, ev.clientY, ev);
});

canvas.addEventListener('touchstart', (ev) => {
  if (ev.cancelable) ev.preventDefault();
  for (const t of ev.changedTouches){
    beginPointer(t.identifier, t.clientX, t.clientY);
  }
  if (ev.touches[0]) updateHoverFromPoint(ev.touches[0].clientX, ev.touches[0].clientY);
}, { passive:false });
canvas.addEventListener('touchmove', (ev) => {
  if (ev.cancelable) ev.preventDefault();
  for (const t of ev.changedTouches){
    movePointer(t.identifier, t.clientX, t.clientY);
  }
  if (!gesture.dragging && !gesture.pinch && ev.touches[0]){
    updateHoverFromPoint(ev.touches[0].clientX, ev.touches[0].clientY);
  }
}, { passive:false });
canvas.addEventListener('touchend', (ev) => {
  if (ev.cancelable) ev.preventDefault();
  for (const t of ev.changedTouches){
    endPointer(t.identifier, t.clientX, t.clientY, { changedTouches:[t], touches:ev.touches, cancelable:false, preventDefault(){} });
  }
}, { passive:false });
canvas.addEventListener('touchcancel', (ev) => {
  for (const t of ev.changedTouches){ gesture.pointers.delete(t.identifier); }
...
function initGame(){
  ...
  if (pf){
    ...
    camera.cy = camera.targetCy = pf.y * TILE_SIZE + TILE_SIZE/2;
  }
